...
int runVS(AParams* PARAMS){
  ...
  // an int goes to over 2 billion, need less than a million, just use int
  for(int i = 0; i < PARAMS->sizePopulation; i++){
    ...
    mingMap[i] = floor(rand_frac() * 100);// a person mingles with
  ...
  for(int i = 0; i < numInitialInfections; i++){
    int randPerson = floor(rand_frac()*1024*1024);
  ...
  for(int day = 0; day < 3650; day++){
     ...
     for(long i=0; i<2500000; i++); // burn some cycles to slow animation

	   cudaMemcpy(stageMap, P1.stage, P1.memIntSize, cD2H);
     numDead = 0;
     numInfected = 0;
     for (int i = 0; i < PARAMS->sizePopulation; i++){
