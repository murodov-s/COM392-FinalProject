...
//  if(argc<2){usage(); return 1;} // must be at least one arg (fileName)
	while((ch = crack(argc, argv, "r|v|f|s|d|", 0)) != NULL) {
	  switch(ch){
    	case 'r' : PARAMS.runMode = atoi(arg_option); break;
    	case 's' : PARAMS.spreadrate = atof(arg_option); break;
      case 'v' : PARAMS.verbosity = atoi(arg_option); break;
			case 'd' : PARAMS.duration = atoi(arg_option); break;
...
int setDefaults(AParams *PARAMS){
    ...
    PARAMS->size      = 1024*1024*3; // 800x800 pixels x 3 colors
	  ...
	  PARAMS->duration = 100; // number of days a person is infectious before symptoms
	  PARAMS->spreadrate = 0.15;	// 0 to 1 to make it faster
